// ============================================================
// Skit Room — Arduino UNO RS485 Responder
// Waits for Mega poll request, reads DHT22, sends reply
// Protocol: Mega sends "GET:SKIT\n", UNO replies "SKIT:21.5,45.2\n"
// ============================================================
...
void setup() {
  ...
  Serial.println(F("Skit Room UNO Ready — poll mode"));
}
...
void respondToMega() {
  ...
  if (isnan(temp) || isnan(humid) || temp <= -40.0 || humid < 0.0) {
    snprintf(packet, sizeof(packet), "SKIT:ERR,ERR\n");
  } else {
    char tempStr[8];
    char humidStr[8];
